...
pub fn start_monitors(pool: &deadpool_redis::Pool, config: &ServerConfig) {
    ...
    start_expiry_monitor(pool.clone(), config.expiry_check_interval.0, config.expiry_check_statuses.clone());
}
...
fn start_expiry_monitor(pool: deadpool_redis::Pool, check_interval: Duration, expiry_check_statuses: Vec<job::Status>) {
    info!(
